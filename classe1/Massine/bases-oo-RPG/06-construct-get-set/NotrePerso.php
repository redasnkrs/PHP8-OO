...
class NotrePerso{
    ...
    public function __construct(?int $lid,?string $lename,?string $espece_perso){
        // utilisation des setters pour protéger les champs
        $this->setName($lename);
        $this->setId($lid);
        $this->setEspecePerso($espece_perso);
    ...
    public function setName(?string $name):void
    {
        if(!is_null($name)){
            ...
            if(empty($name)){
                throw new Exception("Veuillez remplir le champ", 1);
            }
            $this->name = $name;
        }
    ...
    public function setEspecePerso(?string $race):void
    {
        ...
        throw new Exception("Espece inexistante", 1);
