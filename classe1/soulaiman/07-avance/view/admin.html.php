...
<div class="container-content mt-5">
    ...
    <?php if (empty($nosArticle)): ?>
        <h4 class="text-center text-muted">Pas encore d'articles sur notre site</h4>
    <?php else: ?>
        <?php
        $nbArticle = count($nosArticle);
        $pluriel = $nbArticle > 1 ? "s" : "";
        ?>
        <h5>Il y a <?=$nbArticle?> article<?=$pluriel?></h5>
            ...
            <table class="table table-bordered align-middle">
                ...
                <?php foreach ($nosArticle as $item): ?>
                    <tr>
                        ...
                        <td><a href="?p=update&id=<?=$item->getId()?>" class="btn-edit">Modifier</a></td>
                        <td><a href="#" class="btn-delete" onclick="if(confirm('Voulez-vous vraiment supprimer l\'article <?=$item->getArticleSlug()?> ?')){ document.location ='?p=delete&id=<?=$item->getId()?>'; }">Supprimer</a></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
